...
export default function MovieDetail({ navigation, route }) {
  ...
  const favorites = useSelector(state => Array.isArray(state.favorites) ? state.favorites : []);
  ...
  const handlePlay = () => navigation.navigate('VideoPlayer');
  ...
  const renderBackdrop = () => {
    ...
    return (
      <View style={styles.backdropContainer}>
        ...
        <LinearGradient colors={['transparent', '#0a0a0a']} style={styles.backdropOverlay} />
  ...
  const renderMetaInfo = () => (
    <View style={styles.metaRow}>
      ...
      <View style={styles.ratingBadge}>
        ...
        <Text style={styles.ratingText}>{Number(movie.vote_average || 0).toFixed(1)} / 10</Text>
      </View>
    </View>
  );
  ...
  const renderActionButtons = () => (
    <View style={styles.actionRow}>
      ...
      <TouchableOpacity style={styles.myListButton} onPress={handleToggleFavorite}>
        <Ionicons name={isFavorite ? 'checkmark' : 'add'} size={20} color="white" />
        <Text style={styles.myListButtonText}>{isFavorite ? 'In My List' : 'My List'}</Text>
      </TouchableOpacity>
    </View>
  );
  ...
  const renderCast = () => {
    ...
    const topCast = Array.isArray(castData) ? castData.slice(0, 15) : [];
  ...
  const renderSimilarMovies = () => {
    const similarData = movie.similar?.results;
    const similarMovies = Array.isArray(similarData) ? similarData.slice(0, 15) : [];
